...
# ***每次启动必须修改的地方***
    ...
    def check_firewalld(self,cmd):
        ...
        if result == []:
            ...
            result1 = re.findall('3306|30080|8080|6379|15672|5672|8123|443|5005|27017|7000', str(Ssh().connect('firewall-cmd --list-all')), re.S)
            ...
            if len(result1) > 10:
                ...
                print('防火墙已开放端口有：' + str(result1))
                ports = ['3306', '30080', '8080', '6379', '15672', '5672', '8123', '443','5005','27017','7000']
                for port in ports:
                    i = 0
                    if result1==[]:
    ...
    def check_mongodb(self  ,cmd):
        ...
        if result == []:
            ...
            print("开启mongodb中\n")
            Ssh().connect('cd /usr/local/mongodb/bin && ./mongod --config /usr/local/mongodb/conf/mongodb.conf')
            return 1
...
### 后续需要修改 ###
            if result_test == 0:
                ...
                self.ssh_con.exec_command(' cd /home/fortbox/core/asi/base && mv nohup.out '+date+'_nohup.out')
                self.ssh_con.exec_command( 'cd /home/fortbox/core/asi/base && nohup java -jar -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005 asi-allinone-RELEASE-2.3.3-execute.jar  &')
